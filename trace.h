...
void wait_and_exit_if_alien_exited() {
    ...
    if (WIFEXITED(status) || WIFSIGNALED(status)) {
        err_simple_message("Alien process exited unexpectedly.");
    }
}
...
void process_print(pid_t pid, struct user_regs_struct *regs) {
    ...
    copy_to_host(pid, (void *) local_copy, (void *) regs->rdx, regs->r10);
    emulate_print(regs->rsi, regs->rdi, local_copy, regs->r10);
    free(local_copy);
}

void execute(const char *path, struct section_info params_section, const int *params_vals) {
    pid_t sandbox_pid = fork();
    err_execution_failure_conditional(-1 == sandbox_pid,
                                      "Error in fork.");
    if (sandbox_pid == 0) {
        ptrace(PTRACE_TRACEME, 0, NULL, NULL);
        char *const args[1] = { path };
        ...
        ptrace(PTRACE_SYSCALL, sandbox_pid, NULL, NULL); // start of execve syscall
        wait_and_exit_if_alien_exited();
        ptrace(PTRACE_SYSCALL, sandbox_pid, NULL, NULL); // end of execve syscall
        wait_and_exit_if_alien_exited();
        // execve finished, we set program parameters:
